...
GET_CLIENTS_QUERY = '''SELECT username, api_key, news_fingerprint FROM clients'''
GET_DEVICES_QUERY = '''SELECT token, lang, filter FROM devices WHERE username = ?'''
GET_LAST_NEWS_QUERY = '''SELECT hash, type FROM news WHERE username = ? ORDER BY date DESC'''
...
UPDATE_FINGERPRINT_QUERY = '''UPDATE clients SET news_fingerprint = ? WHERE username = ?'''
...
DELETE_OLDEST_NEWS_QUERY = '''DELETE FROM news WHERE id = (SELECT id FROM news WHERE username = ? ORDER BY date ASC LIMIT 1)'''
